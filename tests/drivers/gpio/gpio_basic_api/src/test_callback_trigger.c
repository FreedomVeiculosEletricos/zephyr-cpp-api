...
static void callback(const struct device *dev, struct gpio_callback *gpio_cb,
                     uint32_t pins) {
  const struct drv_data *dd = CONTAINER_OF(gpio_cb, struct drv_data, gpio_cb);
...
ZTEST(gpio_port_cb_vari, test_gpio_callback_variants) {
  ...
  zassert_equal(test_callback(GPIO_INT_LEVEL_HIGH), TC_PASS,
                "level high failed");
  zassert_equal(test_callback(GPIO_INT_LEVEL_LOW), TC_PASS, "level low failed");
  ...
  zassert_equal(test_callback(GPIO_INT_EDGE_BOTH), TC_PASS, "edge both failed");
}
